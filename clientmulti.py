...
"""
THIS PART DIFFERS ON MULTITHREAD

This is is because I need to take the port given by the server, and connect to the server on that port to recieve the
data. I will simply hail the server, then hail the socket with the request on the port given.
"""


# After sending a message to the server, I expect to get the port i need to connect to in response
new_port = int(clientSocket.recv(1024).decode())
print(new_port)

# Since I don't need this connection anymore, its disconnected.
clientSocket.close()

# Connect to the new port assigned by the server
new_socket = socket(AF_INET, SOCK_STREAM)
new_socket.connect((SERVER_ADDRESS, new_port))
print("connected to server")

# Send the HTTP request message to the new socket, the encode method turns it into bytes
new_socket.send(request.encode())
print("Sent Message")


"""THIS PART MARKS THE END OF DIFFERENCE ON MULTITHREAD"""
...
response = ''
while True:
    data = new_socket.recv(BUFFER_SIZE).decode()
    if not data:
        break
    response += data
